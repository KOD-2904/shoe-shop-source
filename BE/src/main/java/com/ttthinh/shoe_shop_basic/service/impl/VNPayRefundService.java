...
public class VNPayRefundService {
    ...
    public void refundFull(Payment payment, String reason) {
        ...
        body.put("vnp_TxnRef", payment.getOrder().getId());
        ...
        try {
            ...
            if (rawQuery != null) {
                for (String parameter : rawQuery.split("&")) {
                    ...
                    if (parts.length == 2 && "vnp_CreateDate".equals(parts[0])) {
                        return URLDecoder.decode(parts[1], StandardCharsets.UTF_8);
                    }
                }
            }
        } catch (Exception exception) {
            log.warn("Cannot read original vnp_CreateDate for paymentId={}", payment.getId());
        }
