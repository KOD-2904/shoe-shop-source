...
public class VNPayService {
    ...
    public String createPaymentUrl(VNPayPaymentRequest request, HttpServletRequest httpServletRequest) {
        validateConfig();
        String amountInCents = request.getAmount().multiply(BigDecimal.valueOf(100)).toBigIntegerExact().toString();

        String txnRef = StringUtils.hasText(request.getTxnRef())
                ? request.getTxnRef()
        ...
        params.put("vnp_OrderInfo", normalizeOrderInfo(request.getOrderInfo())); // Bỏ dấu tiếng Việt
    ...
    private void validateConfig() {
        if (!StringUtils.hasText(vnPayConfig.getTmnCode())
                ...
                || !StringUtils.hasText(vnPayConfig.getPaymentUrl())
                || !StringUtils.hasText(vnPayConfig.getReturnUrl())) {
            throw new IllegalStateException("Missing VNPay configuration");
        }
        if (!vnPayConfig.getReturnUrl().startsWith("https://")) {
            log.warn("VNPay returnUrl should be HTTPS for public sandbox/prod: {}", vnPayConfig.getReturnUrl());
        }
        if (StringUtils.hasText(vnPayConfig.getIpnUrl()) && !vnPayConfig.getIpnUrl().startsWith("https://")) {
            log.warn("VNPay ipnUrl should be HTTPS for public sandbox/prod: {}", vnPayConfig.getIpnUrl());
        }
    }

    private String normalizeOrderInfo(String text) {
        String source = StringUtils.hasText(text) ? text : "Thanh toan don hang";
    ...
    public boolean matchesTmnCode(String tmnCode) {
        return StringUtils.hasText(tmnCode) && tmnCode.equals(vnPayConfig.getTmnCode());
    }
